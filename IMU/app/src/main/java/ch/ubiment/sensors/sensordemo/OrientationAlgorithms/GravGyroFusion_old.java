...
import ch.ubiment.sensors.sensordemo.Algebra.Matrix3;
import ch.ubiment.sensors.sensordemo.Algebra.Quaternion;

public class GravGyroFusion_old {
     ...
     * @return
     */
    private Quaternion alignWithWorldUp(Quaternion orientation, float[] world_up){

        // get the up vector of the orientation

        float[] orient_up = {0, 0, 1};
        if (false){
            // rotate with quaternion
            orient_up = normalized(orientation.rotateVector(orient_up));
        } else {
            if (false){
                // jst a test
                Matrix3 Rmat = new Matrix3(orientation);
                orient_up = Rmat.multiply(orient_up);
                double[] rv = {0,1,0};
                Quaternion quat = new Quaternion(3.1416/4, rv);
            }
            // rotate with matrix
            Matrix3 Rmat = new Matrix3(orientation);
            orient_up = Rmat.transposed().multiply(orient_up);
        }


        // The angle of rotation is acos( d ), where d is the dot product of orient_up and world_up (both normalized).
        double dot = orient_up[0]*world_up[0] + orient_up[1]*world_up[1] + orient_up[2]*world_up[2];
        double radian = Math.acos(dot);
        // the rotation axis is the vectorial product of orient_up with world_up
        double[] rotation_axis = normalized(crossProduct3(orient_up, world_up));

        // computes the quaternion aligning orientation to world_up
        Quaternion deltaRotation =  (new Quaternion(radian, rotation_axis)).normalized();

        Quaternion Qout = (orientation.normalized().times(deltaRotation.normalized())).normalized();
        return Qout;
    }
     ...
     * @return
     */
    private double[] crossProduct3(float[] u, float[] v){
        double[] out = {
                (double)( u[1]*v[2] - u[2]*v[1]),
                (double)(-u[0]*v[2] + u[2]*v[0]),
     ...
     * @return
     */
    public Quaternion update(float[] gravity, float[] angularRate_xyz, float new_angular_ts){
        ...
        if (!positionInitialised) {
            absoluteOrientationQuaternion = alignWithWorldUp(new Quaternion(), world_up);

            relativeOrientationQuaternion = absoluteOrientationQuaternion.copy();
        ...
        absoluteOrientationQuaternion = alignWithWorldUp(relativeOrientationQuaternion, world_up);

        if (false){ // thats the supposly real version, not working yet
            // estimate fusion
    ...
    private Quaternion estimateFusion(){
        ...
        if (Math.abs(dotProd) < OUTLIER_THRESHOLD) {
            ...
            float t = (float) (INDIRECT_INTERPOLATION_WEIGHT * angularVelocity);
            output = relativeOrientationQuaternion.slerp(absoluteOrientationQuaternion, t);
            Log.d(TAG, "dotProd: " + dotProd + " \tt: " + t);
